 ...
 */
public class NoteDisplay extends Panel {
	public final static String TITLE = "Quantity of Notes Available";
	 ...
	 */
	public NoteDisplay(MaintenanceController mctrl) {
		mCtrl = mctrl;
		storeCtrl = mCtrl.getMainController().getStoreController();

		len = storeCtrl.getStoreSize(Store.NOTE);
		StoreItem[] items = storeCtrl.getStoreItems(Store.NOTE);

		bi = new ButtonItemDisplay(TITLE, items, len);

		bi.addListener(new NoteDisplayListener(mCtrl));
